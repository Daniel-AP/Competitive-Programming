...
void sieve(const int N, const int Q = 17, const int L = 1 << 15) {
    ...
    for (size_t pi = 0; pi < pbeg; ++pi) {
        P pp = sprimes[pi]; const int p = pp.p;
        for (int t = 0; t < 8; ++t) {
    ...
    vector<unsigned char> block(block_size); unsigned char* pblock = &block[0];
    const int M = (N + 29) / 30;

    for (int beg = 0; beg < M; beg += block_size) {
        int end = min(M, beg + block_size);
        for (int i = beg; i < end; i += prod) {
            copy(pre.begin(), pre.end(), &block[0] + (i - beg));
        }
...
void solve() {
    ...
    for(int i = 1; i <= primes.size(); i += 500) cout << primes[i-1] << '\n';
