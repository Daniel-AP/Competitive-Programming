...
struct Lazy {
    ...
    bool is_zero() { return v == 1; }
};
struct Mono {
    int v;
    static Mono zero() { return {0}; }
    void apply_lazy(Lazy d, int l, int r) { v *= d.v; v %= MOD; }
};
Lazy operator+ (Lazy a, Lazy b) { return {a.v*b.v%MOD}; }
Mono operator+ (Mono a, Mono b) { return {(a.v+b.v)%MOD}; }
struct SegTreeLazy {
    int len;
    vector<Mono> data;
    vector<Lazy> lazy;
